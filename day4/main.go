...
func (l LogEntry) key() string {
	return l.date + l.hour + l.min
}

type GuardLogs map[string]*mins

// assume fixed size till comment
...
func parseLog(s string) LogEntry {
	...
	l.hour = s[12:14]
	l.min = s[15:17]
	comment := strings.Split(s[19:], " ")
	if comment[0] == "Guard" {
		l.guardID = comment[1]
		l.event = strings.Join(comment[2:], " ")
	} else {
		l.event = strings.Join(comment[:], " ")
	}
	return l
}
...
// given a sorted event log divide up which guard had the shift and when they slept
func captureShifts(logs []LogEntry) GuardLogs {
	// guard id to mins total
	guardLogs := GuardLogs{}

	wokeUp := func(guard, date string, sleepTime int, woke int) {
		//fmt.Printf("Guard [%v] woke up on [%v] - slept [%v - %v]: [%v] mins\n", guard, date, sleepTime, woke, (woke - sleepTime))
		times, ok := guardLogs[guard]
		if !ok {
			times = &mins{}
			guardLogs[guard] = times
		}
		for i := sleepTime; i < woke; i++ {
			times[i] += 1
		}
	}
	...
	for _, l := range logs {
		if l.date != currentDate {
			// if asleep at the end of a shift
			if isAsleep {
				wokeUp(currentGuard, currentDate, sleepTime, 59)
				isAsleep = false
			}
			currentDate = l.date
			//fmt.Printf("Day [%s], guard [%s] is sleeping: %v\n", currentDate, currentGuard, isAsleep)
		}

		switch l.event {
		case "begins shift":
			currentGuard = l.guardID
			//fmt.Printf("Guard %v: started at %v\n", currentGuard, l.min)
		case "falls asleep":
			sleepTime = num(l.min)
		case "wakes up":
			wokeUp(currentGuard, currentDate, sleepTime, num(l.min))
			sleepTime = 0
		default:
			panic(fmt.Sprintf("unknown event: %v", l.event))
		}
	}
...
func daySleptMost(times *mins) (minute, count int) {
	for i, c := range times {
		if c > count {
			count = c
			minute = i
...
func main() {
	...
	guard := findMaxSleeper(guardLogs)
	fmt.Printf("Guard that sleeps the most: %v\n", guard)

	min, count := daySleptMost(guardLogs[guard])
